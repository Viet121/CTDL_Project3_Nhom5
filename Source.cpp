...
//ham hien thi man hinh menu
void Menu();

//ham quan ly sach
void Quanlysach();

//ham chon chuc nang cua quan ly sach
void duyetQuanLySach();

//ham xem toan bo sach co trong thu vien
void Xemtoanbosach();

//ham them 1 cuon sach vao trong thu vien
void ThemSach();

//ham xoa 1 cuon sach co trong thu vien 
void XoaSach();

//ham quay tro ve cac chuc nang cua quan ly sach
void backQuanlysach();

// ham quay tro ve man hinh menu
void backMenu();

// ham quan ly phieu muon
void Quanlyphieumuon();

//ham chon cac chuc nang cua quan ly phieu muon
void duyetQuanlyphieumuon();

//ham quay tro ve cac chuc nang cua quan ly phieu muon 
void backQuanlyphieumuon();

// ham xem toan bo phieu muon
void xemToanbophieumuon();

// ham muon 1 cuon sach co trong thu vien dddddddd
void muonSach();

//ham tra sach da muon
void traSach();

// ham cap nhat lai tinh trang sach sau khi muon hoac tra
...
void XoaSach(){
    ...
    while (!FileIn.eof()){
        ...
        else if (x->getmasach() == MSxoa && x->gettinhtrangsach()==1) {
            FileOut << x->getmasach() << endl << x->gettensach() << endl << x->gettacgia() << endl << x->getnhaxuatban() << endl << x->getgiaban() << endl << x->getnamphathanh() << endl << x->getsotrang() << endl << x->getngaynhapkho() << endl << x->gettinhtrangsach();
            n++;
        }
    }
    ...
    if (n == 0) {
        ...
        textColor(4);
        cout << "Sach da duoc muon, nen khong the xoa !!!" << endl;
        textColor(15);
    }
    backQuanlysach();
}
